...
Note that strings are *case sensitive* - this applies to the input vector of text data _and_ any other string patterns passed to `stringr` functions.
...
Note: if we hadn't used the "or" operator and just passed a _vector_ of vowels to count for, only the first one detected in each string would be counted (e.g. only "a"'s would be counted in "apple pie", because it is the first vowel detected in that string).
...
As you can see, for those strings with two words, the specified pattern extracts the _first_ match of one or more letters which is of course detected in the first word. The anchor to the end of the string is what tells the function to look for the match in the final word of that string.
